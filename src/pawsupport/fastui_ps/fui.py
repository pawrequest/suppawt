...
class Text(c.Text):
    @classmethod
    def all_text(cls, *objs: BaseModel, with_keys: Literal["NO", "YES", "ONLY"] = "NO") -> "list[Text]":
        txts = []
        for obj in objs:
            for k, v in obj.model_dump().items():
                ...
                #     return cls.all_text(*v, with_keys=with_keys)
                if isinstance(v, str):
                    txt_str = ""
                    if with_keys == "NO":
                        txt_str = v
...
class DivPR(c.Div):
    ...
    def list_of(
        ...
        *components: AnyComponent,
        ...
        divs = [cls(components=[comp], class_name=style) for comp in components]
        return divs
...
class PagePR(c.Page):
    ...
    def default_page(
        ...
        *components: AnyComponent,
        ...
        if contained:
            comps = [Container(components=comps)]
        return [
